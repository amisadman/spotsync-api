...
>
> _A centralized platform for busy airports and malls to manage parking zones, specifically handling the high-demand reservation of limited EV charging spots._
...
**Success Response (200 OK)**
_(Returns the same structure as a single item in the list above, including `available_spots`)_
...
> **The Race Condition:** If two drivers try to reserve the very last EV spot at the _exact same millisecond_, both requests might read "19 active" and both will succeed, resulting in 21 cars in a 20-car zone.
...
> 💡 **Hint:** Drivers can only cancel their _own_ reservations. If they try to cancel someone else's, return 403 Forbidden. Changing the status to `cancelled` effectively frees up the spot in the zone.
...
**Success Response (200 OK)**
_(Returns an array of all reservations in the system, including the `user` details and `zone` details via GORM Preloads)._
